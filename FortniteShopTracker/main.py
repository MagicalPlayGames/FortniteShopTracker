...
class MyImage(FitImage):
    ...
    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos) and len(self.item)>1:
            self.itemSelected+=1
...
class MyApp(MDApp):
    ...
    def changeLayout(self,text):
        ...
        if(text=="All"):
            for items in self.itemDict.values():
                self.generateBoxes(items)
        else:
        ...
        if 'items' in item:
            for itemImages in item['items']:
                if 'images' in itemImages:
                    ...
                    if itemImages['variants'] is not None:
                        for variants in itemImages['variants']:
                            if variants['options'] is not None:
    ...
    def splitItems(self,items):
        for item in items :
            ...
            if layoutName is None:
                layoutName = "Other"
            if 'layoutId' in item:
                if 'Jam' in item['layoutId'] and item['tracks'] is not None:
        ...
        for aType in accountTypes:
           ...
           try:
            ...
            if response.json()['status']==200:
               ...
               print("Request timedout during findStats function")
               continue
...
def getResponse(url):
    ...
    try:
        ...
        print("Program has timed out while Authenticating")
    except requests.RequestException as e:
        print(e)
        Internet=False
